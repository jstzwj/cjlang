...
def emit_table(f, name, t_data, t_type = "List[Tuple[str, str]]", is_pub=True,
...
def emit_property_module(f, mod, tbl, emit):
    for cat in sorted(emit):
        ...
        f.write("def %s(c: str) -> bool:\n" % cat)
        f.write("    return bsearch_range_table(c, %s_table)\n" % cat)
        f.write("\n\n")
    f.write("\n\n")
...
if __name__ == "__main__":
    ...
    if os.path.exists(r):
        os.remove(r)
    with open(r, "w", encoding="utf-8") as rf:
...
UNICODE_VERSION: Tuple[int, int, int] = (%s, %s, %s)
""" % unicode_version)
